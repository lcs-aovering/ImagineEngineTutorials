...
class AsetroidBlasterScene: Scene {
    override func setup()  {
        backgroundColor = Color(red: 0, green: 0, blue: 0.3, alpha: 1.0)
        
        let groundSize = Size(width: size.width, height: 100)
        let ground = Block(size: groundSize,textureCollectionName: "Ground");add(ground)
...
let scene = AsetroidBlasterScene(size: sceneSize)
